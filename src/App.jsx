...
const GlobalStyle = () => (
  <style>{`
    /* [보안 수정 4번] font-display:swap 추가 — 폰트 로딩 지연/차단 시 시스템 폰트로 즉시 렌더링 */
    @import url('https://fonts.googleapis.com/css2?family=Nunito:wght@400;600;700;800;900&family=Noto+Sans+JP:wght@400;500;700&display=swap&font-display=swap');

    * { box-sizing: border-box; margin: 0; padding: 0; }

    body {
      /* [보안 수정 4번] 시스템 폰트 폴백 추가 — CDN 차단/지연 시 Apple/Windows/Android 기본 폰트로 렌더링 */
      font-family: 'Nunito', 'Noto Sans JP', -apple-system, BlinkMacSystemFont, 'Segoe UI', 'Apple SD Gothic Neo', sans-serif;
      background: #EEF2FF;
      min-height: 100vh;
    }
...
export default function App() {
  ...
  // [보안 수정] 허용할 문자만 통과시키는 정규식 (한글, 일본어, 영숫자, 공통 문장부호만 허용)
  const ALLOWED_CHARS = /^[\p{Script=Hiragana}\p{Script=Katakana}\p{Script=Han}\p{Script=Hangul}A-Za-z0-9 .,!?~\-_'":;()。、！？〜ー～…]+$/u;

  // [보안 수정] 각 QA 쌍의 유효성을 검사하는 함수
  const isValidQaPair = (pair) => {
    // 1. 객체 구조 확인: id, input, output 필드만 허용
    if (typeof pair !== 'object' || pair === null) return false;
    const allowedKeys = new Set(['id', 'input', 'output']);
    if (!Object.keys(pair).every(k => allowedKeys.has(k))) return false;

    // 2. 타입 확인: input, output은 반드시 문자열
    if (typeof pair.input !== 'string' || typeof pair.output !== 'string') return false;

    // 3. 길이 제한: 앱 내 입력 제한(15자)과 동일하게 적용
    if ([...pair.input].length > MAX_INPUT_LEN || [...pair.output].length > MAX_INPUT_LEN) return false;

    // 4. 빈 값 방지
    if (!pair.input.trim() || !pair.output.trim()) return false;

    // 5. 허용 문자 검사: HTML 특수문자(<, >, &, ", ') 및 스크립트 삽입 차단
    if (!ALLOWED_CHARS.test(pair.input) || !ALLOWED_CHARS.test(pair.output)) return false;
  ...
  useEffect(() => {
    ...
    // [보안 수정] URL 파라미터 최대 길이 제한 (비정상적으로 큰 데이터 차단)
    if (data.length > 5000) return;

    try {
      const decoded = JSON.parse(decodeURIComponent(atob(decodeURIComponent(data))));

      // [보안 수정] 배열 여부 및 규칙 수 상한(MAX_RULES) 검사
      if (!Array.isArray(decoded) || decoded.length === 0 || decoded.length > MAX_RULES) return;

      // [보안 수정] 모든 QA 쌍을 개별 검증 — 하나라도 실패하면 전체 거부
      if (!decoded.every(isValidQaPair)) return;

      // 검증 통과 시에만 상태 반영
      const safePairs = decoded.map((pair, i) => ({ ...pair, id: i + 1 }));
  ...
  const copyToClipboard = async () => {
    if (!shareLink) return;
    try {
      // [보안 수정 3번] deprecated execCommand 대신 공식 Clipboard API 사용
      await navigator.clipboard.writeText(shareLink);
      setCopied(true);
      setTimeout(() => setCopied(false), 2500);
    } catch {
      // [보안 수정 5번] console.error 제거 — 에러 정보 외부 노출 방지
      // Clipboard API 미지원 환경(구형 브라우저) 대비 폴백
      try {
